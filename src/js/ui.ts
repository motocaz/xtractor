...
export const showLoginModal = () => {
    ...
    newButton.addEventListener('click', () => {
        window.location.href = `${import.meta.env.BASE_URL}src/pages/sign-in.html`;
    });
...
export const renderPageThumbnails = async (toolId: any, pdfDoc: any) => {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
        ...
        imgContainer.className =
            'w-full h-36 bg-(--background) rounded-lg flex items-center justify-center overflow-hidden border-2 border-gray-600';
...
export const renderFileDisplay = (container: any, files: any) => {
    ...
    if (files.length > 0) {
        files.forEach((file: any) => {
            ...
            fileDiv.className =
                'flex items-center justify-between bg-input-bg p-3 rounded-lg text-sm';
...
const createFileInputHTML = (options = {}) => {
    ...
    return `
        <div id="drop-zone" class="relative flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-fuchsia-700 rounded-xl cursor-pointer bg-(--background) hover:bg-tool-card transition-colors duration-300">
        ...
        ${showControls
            ...
            <div id="file-controls" class="hidden mt-4 flex gap-3">
                <button id="add-more-btn" class="btn bg-fuchsia-600 hover:bg-fuchsia-700 text-white font-semibold px-4 py-2 rounded-lg flex items-center gap-2">
...
export const toolTemplates = {
    ...
    <div id="merge-options" class="hidden mt-6">
        <div class="flex gap-2 p-1 rounded-lg bg-(--background) border border-gray-700 mb-4">
            <button id="file-mode-btn" class="flex-1 btn bg-fuchsia-600 text-white font-semibold py-2 rounded-md">File Mode</button>
        ...
        <div id="file-mode-panel">
            <div class="p-3 bg-(--background) rounded-lg border border-gray-700 mb-3">
        ...
        <div id="page-mode-panel" class="hidden">
             ...
             <div id="page-merge-preview" class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-4 p-4 bg-(--background) rounded-lg border border-gray-700 min-h-[200px]"></div>
        </div>
...
`,
    ...
    <div id="split-options" class="hidden mt-6">
        ...
        <select id="split-mode" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5 mb-4">
        ...
        <div id="range-panel">
            ...
            <input type="text" id="page-range" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5" placeholder="e.g., 1-5, 8">
        </div>
        ...
        <div id="even-odd-panel" class="hidden">
            ...
            <div class="flex gap-4">
                <label class="flex-1 flex items-center justify-center gap-2 p-3 rounded-md hover:bg-gray-700 cursor-pointer has-checked:bg-fuchsia-600">
        ...
        <div id="bookmarks-panel" class="hidden">
            ...
            <div class="mb-4">
                ...
                <select id="bookmark-level" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
        ...
        <div id="n-times-panel" class="hidden">
            ...
            <div class="mb-4">
                ...
                <input type="number" id="split-n-value" min="1" value="5" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
        ...
        <div id="zip-option-wrapper" class="hidden mt-4">
            <label class="flex items-center gap-2 text-sm font-medium text-gray-300">
                <input type="checkbox" id="download-as-zip" class="w-4 h-4 rounded text-fuchsia-600 bg-gray-700 border-gray-600 focus:ring-fuchsia-500">
...
`,
  ...
  <div id="encrypt-options" class="hidden space-y-4 mt-6">
      <div>
          ...
          <input required type="password" id="user-password-input" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5" placeholder="Password to open the PDF">
      ...
      <div>
          ...
          <input type="password" id="owner-password-input" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5" placeholder="Password for full permissions (recommended)">
...
`,
    decrypt: () => `
        ...
        <div id="decrypt-options" class="hidden space-y-4 mt-6">
            <div>
                ...
                <input type="password" id="password-input" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5" placeholder="Enter the current password">
            </div>
    ...
    rotate: () => `
        ...
        <div id="rotate-all-controls" class="hidden my-6">
            <div class="bg-input-bg border border-gray-700 rounded-lg p-4">
                <h3 class="text-sm font-semibold text-gray-400 mb-3 text-center">BATCH ACTIONS</h3>
                <div class="flex justify-center gap-4">
                    <button id="rotate-all-left-btn" class="flex items-center justify-center w-full px-4 py-2 text-sm font-medium text-gray-200 bg-tool-card border border-gray-600 rounded-lg shadow-sm hover:bg-input-bg hover:border-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 focus:ring-fuchsia-500 transform transition-all duration-150 active:scale-95">
                        <i data-lucide="rotate-ccw" class="mr-2 h-4 w-4"></i>
                        Rotate All Left
                    </button>
                    <button id="rotate-all-right-btn" class="flex items-center justify-center w-full px-4 py-2 text-sm font-medium text-gray-200 bg-tool-card border border-gray-600 rounded-lg shadow-sm hover:bg-input-bg hover:border-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 focus:ring-fuchsia-500 transform transition-all duration-150 active:scale-95">
    ...
    'add-page-numbers': () => `
        ...
        <div id="pagenum-options" class="hidden grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
            <div>
                ...
                <select id="position" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            ...
            <div>
                ...
                <input type="number" id="font-size" value="12" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            </div>
            <div>
                <label for="number-format" class="block mb-2 text-sm font-medium text-gray-300">Format</label>
                <select id="number-format" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
    ...
    'jpg-to-pdf': () => `
        ...
        <div id="jpg-to-pdf-options" class="hidden mt-6">
            <div class="mb-4">
                ...
                <select id="jpg-pdf-quality" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
...
`,
    ...
    <div id="compress-options" class="hidden mt-6 space-y-6">
        <div>
            ...
            <select id="compression-level" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5 focus:ring-fuchsia-500 focus:border-fuchsia-500">
        ...
        <div>
            ...
            <select id="compression-algorithm" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5 focus:ring-fuchsia-500 focus:border-fuchsia-500">
...
`,
    ...
    <div class="p-3 mb-6 bg-input-bg border border-yellow-500/30 text-yellow-200/80 rounded-lg text-sm flex items-start gap-3">
    ...
    <div id="metadata-form" class="hidden mt-6 space-y-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" id="meta-title" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            </div>
            <div>
                <label for="meta-author" class="block mb-2 text-sm font-medium text-gray-300">Author</label>
                <input type="text" id="meta-author" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            </div>
            <div>
                <label for="meta-subject" class="block mb-2 text-sm font-medium text-gray-300">Subject</label>
                <input type="text" id="meta-subject" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            </div>
             <div>
                <label for="meta-keywords" class="block mb-2 text-sm font-medium text-gray-300">Keywords (comma-separated)</label>
                <input type="text" id="meta-keywords" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            </div>
            <div>
                <label for="meta-creator" class="block mb-2 text-sm font-medium text-gray-300">Creator Tool</label>
                <input type="text" id="meta-creator" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            </div>
            <div>
                <label for="meta-producer" class="block mb-2 text-sm font-medium text-gray-300">Producer Tool</label>
                <input type="text" id="meta-producer" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            </div>
             <div>
                <label for="meta-creation-date" class="block mb-2 text-sm font-medium text-gray-300">Creation Date</label>
                <input type="datetime-local" id="meta-creation-date" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            </div>
            <div>
                <label for="meta-mod-date" class="block mb-2 text-sm font-medium text-gray-300">Modification Date</label>
                <input type="datetime-local" id="meta-mod-date" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            </div>
        </div>
        ...
        <button id="add-custom-meta-btn" class="btn border border-gray-600 hover:bg-input-bg text-white font-semibold py-2 px-4 rounded-lg flex items-center gap-2">
...
`,
    ...
    'png-to-pdf': () => `
        ...
        <div id="png-to-pdf-options" class="hidden mt-6">
            <div class="mb-4">
                ...
                <select id="png-pdf-quality" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
                    <option value="high">High Quality (Larger file)</option>
                    <option value="medium" selected>Medium Quality (Balanced)</option>
                    <option value="low">Low Quality (Smaller file)</option>
                </select>
    ...
    'delete-pages': () => `
        ...
        <div id="delete-options" class="hidden mt-6">
            ...
            <input type="text" id="pages-to-delete" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5 mb-6" placeholder="e.g., 2, 4-6, 9">
    ...
    'add-blank-page': () => `
        ...
        <div id="blank-page-options" class="hidden mt-6">
            ...
            <input type="number" id="page-number" min="0" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5 mb-4" placeholder="Enter 0 to add to the beginning">
            <label for="page-count" class="block mb-2 text-sm font-medium text-gray-300">Number of blank pages to insert:</label>
            <input type="number" id="page-count" min="1" value="1" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5 mb-6" placeholder="Enter number of pages">
    ...
    'extract-pages': () => `
        ...
        <div id="extract-options" class="hidden mt-6">
            ...
            <input type="text" id="pages-to-extract" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5 mb-6" placeholder="e.g., 2, 4-6, 9">
    ...
    <div id="watermark-options" class="hidden mt-6 space-y-4">
        <div class="flex gap-4 p-2 rounded-lg bg-input-bg">
            <label class="flex-1 flex items-center justify-center gap-2 p-3 rounded-md hover:bg-gray-700 cursor-pointer has-checked:bg-fuchsia-600">
        ...
        <div id="text-watermark-options">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="watermark-text" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5" placeholder="e.g., CONFIDENTIAL">
                </div>
                <div>
                    <label for="font-size" class="block mb-2 text-sm font-medium text-gray-300">Font Size</label>
                    <input type="number" id="font-size" value="72" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
                </div>
            </div>
        ...
        <div id="image-watermark-options" class="hidden space-y-4">
            <div>
                ...
                <input type="file" id="image-watermark-input" accept="image/png, image/jpeg" class="w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-fuchsia-600 file:text-white hover:file:bg-fuchsia-700">
            </div>
            <div>
...
`,
    ...
    <div id="header-footer-options" class="hidden mt-6 space-y-4">
        ...
        <div class="p-4 bg-(--backbround) border border-gray-700 rounded-lg">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" id="page-range" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5" placeholder="e.g., 1-3, 5">
                    <p class="text-xs text-gray-400 mt-1">Total pages: <span id="total-pages">0</span></p>
                </div>
                <div>
                    <label for="font-size" class="block mb-2 text-sm font-medium text-gray-300">Font Size</label>
                    <input type="number" id="font-size" value="10" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
                </div>
                <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <input type="text" id="header-left" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            </div>
            <div>
                <label for="header-center" class="block mb-2 text-sm font-medium text-gray-300">Header Center</label>
                <input type="text" id="header-center" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            </div>
            <div>
                <label for="header-right" class="block mb-2 text-sm font-medium text-gray-300">Header Right</label>
                <input type="text" id="header-right" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <label for="footer-left" class="block mb-2 text-sm font-medium text-gray-300">Footer Left</label>
                <input type="text" id="footer-left" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            </div>
            <div>
                <label for="footer-center" class="block mb-2 text-sm font-medium text-gray-300">Footer Center</label>
                <input type="text" id="footer-center" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            </div>
            <div>
                <label for="footer-right" class="block mb-2 text-sm font-medium text-gray-300">Footer Right</label>
                <input type="text" id="footer-right" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            </div>
        </div>
    </div>
...
`,
    ...
    <div id="permissions-options" class="hidden mt-6 space-y-4">
        <div>
            ...
            <input type="password" id="current-password" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5" placeholder="Leave blank if PDF is not password-protected">
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label for="new-user-password" class="block mb-2 text-sm font-medium text-gray-300">New User Password (optional)</label>
                <input type="password" id="new-user-password" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5" placeholder="Password to open PDF">
            </div>
            <div>
                <label for="new-owner-password" class="block mb-2 text-sm font-medium text-gray-300">New Owner Password (optional)</label>
                <input type="password" id="new-owner-password" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5" placeholder="Password for full permissions">
            </div>
        </div>

        <div class="p-4 bg-(--background) border border-blue-500/30 text-blue-200 rounded-lg">
...
`,
    ...
    'txt-to-pdf': () => `
        ...
        <div class="mb-4">
            <div class="flex gap-2 p-1 rounded-lg bg-(--background) border border-gray-700 mb-4">
                <button id="txt-mode-upload-btn" class="flex-1 btn bg-fuchsia-600 text-white font-semibold py-2 rounded-md">Upload Files</button>
            ...
            <div id="txt-text-panel" class="hidden">
                <textarea id="text-input" rows="12" class="w-full bg-input-bg border border-gray-600 text-gray-300 rounded-lg p-2.5 font-sans" placeholder="Start typing here..."></textarea>
            </div>
        </div>
        ...
        <div class="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
                ...
                <select id="font-family" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            ...
            <div>
                ...
                <select id="page-size" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
    ...
    'view-metadata': () => `
        ...
        ${createFileInputHTML()}
        <div id="file-display-area" class="mt-4 space-y-2"></div>
        <div id="metadata-results" class="hidden mt-6 p-4 bg-input-bg border border-gray-700 rounded-lg"></div>
    `,
    'reverse-pages': () => `
    ...
    'md-to-pdf': () => `
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div>
                ...
                <select id="page-format" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
                    <option value="a4">A4</option>
                    <option value="letter">Letter</option>
                </select>
            </div>
            <div>
                ...
                <select id="orientation" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            ...
            <div>
                ...
                <select id="margin-size" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
    ...
    'split-in-half': () => `
        ...
        <div id="split-half-options" class="hidden mt-6">
            ...
            <select id="split-type" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5 mb-6">
        ...
        <div id="n-up-options" class="hidden mt-6 space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="pages-per-sheet" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
                ...
                <div>
                    ...
                    <select id="output-page-size" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                 <div>
                    ...
                    <select id="output-orientation" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
                ...
                <div class="flex items-end pb-1">
                     <label class="flex items-center gap-2 text-sm font-medium text-gray-300">
                        <input type="checkbox" id="add-margins" checked class="w-4 h-4 rounded text-fuchsia-600 bg-gray-700 border-gray-600 focus:ring-fuchsia-500">
                        Add Margins & Gutters
                    </label>
                </div>
            </div>
            ...
            <div class="border-t border-gray-700 pt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="flex items-center">
                     <label class="flex items-center gap-2 text-sm font-medium text-gray-300">
                        <input type="checkbox" id="add-border" class="w-4 h-4 rounded text-fuchsia-600 bg-gray-700 border-gray-600 focus:ring-fuchsia-500">
        ...
        <div id="combine-options" class="hidden mt-6 space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" id="page-spacing" value="18" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
                </div>
                <div>
            ...
            <div>
                <label class="flex items-center gap-2 text-sm font-medium text-gray-300">
                    <input type="checkbox" id="add-separator" class="w-4 h-4 rounded text-fuchsia-600 bg-gray-700 border-gray-600 focus:ring-fuchsia-500">
        ...
        <div id="fix-dimensions-options" class="hidden mt-6 space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="target-size" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
            ...
            <div>
                ...
                <div class="flex gap-4 p-2 rounded-lg bg-input-bg">
                    <label class="flex-1 flex items-center gap-2 p-3 rounded-md hover:bg-gray-700 cursor-pointer">
                        <input type="radio" name="scaling-mode" value="fit" checked class="w-4 h-4 text-fuchsia-600 bg-gray-700 border-gray-600 focus:ring-fuchsia-500">
                        <div>
                    ...
                    <label class="flex-1 flex items-center gap-2 p-3 rounded-md hover:bg-gray-700 cursor-pointer">
                        <input type="radio" name="scaling-mode" value="fill" class="w-4 h-4 text-fuchsia-600 bg-gray-700 border-gray-600 focus:ring-fuchsia-500">
                         <div>
    ...
    'compare-pdfs': () => `
        ...
        <div id="compare-upload-area" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div id="drop-zone-1" class="relative flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-fuchsia-700 rounded-xl cursor-pointer bg-(--background) hover:bg-tool-card transition-colors duration-300">
            ...
            <div id="drop-zone-2" class="relative flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-fuchsia-700 rounded-xl cursor-pointer bg-(--background) hover:bg-tool-card transition-colors duration-300">
        ...
        <div id="compare-viewer" class="hidden mt-6">
            <div class="flex flex-wrap items-center justify-center gap-4 mb-4 p-3 bg-input-bg rounded-lg border border-gray-700">
                ...
                <div class="bg-gray-700 p-1 rounded-md flex gap-1">
                    <button id="view-mode-overlay" class="btn bg-fuchsia-600 px-3 py-1 rounded text-sm font-semibold">Overlay</button>
                ...
                <div id="side-by-side-controls" class="hidden flex items-center gap-2">
                    <label class="flex items-center gap-2 text-sm font-medium text-gray-300 cursor-pointer">
                        <input type="checkbox" id="sync-scroll-toggle" checked class="w-4 h-4 rounded text-fuchsia-600 bg-gray-700 border-gray-600 focus:ring-fuchsia-500">
                        Sync Scrolling
                    </label>
                </div>
            </div>
    ...
    <div class="p-3 bg-(--background) rounded-lg border border-fuchsia-700 mb-6">
        <p class="text-sm text-gray-300"><strong class="text-white">How it works:</strong></p>
        <ul class="list-disc list-inside text-xs text-gray-400 mt-1 space-y-1">
    ...
    <div id="ocr-options" class="hidden mt-6 space-y-4">
        <div>
            ...
            <div class="relative">
                <input type="text" id="lang-search" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5 mb-2" placeholder="Search for languages...">
                <div id="lang-list" class="max-h-48 overflow-y-auto border border-gray-600 rounded-lg p-2 bg-input-bg">
                    ${Object.entries(tesseractLanguages)
            .map(
                ([code, name]) => `
                        <label class="flex items-center gap-2 p-2 rounded-md hover:bg-gray-700 cursor-pointer">
                            <input type="checkbox" value="${code}" class="lang-checkbox w-4 h-4 rounded text-fuchsia-600 bg-gray-700 border-gray-600 focus:ring-fuchsia-500">
                            ${name}
                        </label>
                    `
            )
        ...
        <details class="bg-input-bg border border-gray-700 rounded-lg p-3">
            ...
            <div class="mt-4 space-y-4">
                ...
                <div>
                    ...
                    <select id="ocr-resolution" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2 text-sm">
                ...
                <label class="flex items-center gap-2 text-sm text-gray-300 cursor-pointer">
                    <input type="checkbox" id="ocr-binarize" class="w-4 h-4 rounded text-fuchsia-600 bg-gray-700 border-gray-600">
                ...
                <div>
                    ...
                    <select id="whitelist-preset" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2 text-sm mb-2">
                ...
                <div>
                    ...
                    <input type="text" id="ocr-whitelist" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2 text-sm" placeholder="e.g., abcdefghijklmnopqrstuvwxyz0123456789$.,">
                    <p class="text-xs text-gray-500 mt-1">Only these characters will be recognized. Leave empty for all characters.</p>
                </div>
            </div>
        </details>
    ...
    <div id="ocr-progress" class="hidden mt-6 p-4 bg-input-bg border border-gray-700 rounded-lg">
        <p id="progress-status" class="text-white mb-2">Initializing...</p>
        <div class="w-full bg-gray-700 rounded-full h-4">
            <div id="progress-bar" class="bg-fuchsia-600 h-4 rounded-full transition-width duration-300" style="width: 0%"></div>
        </div>
    ...
    <div id="ocr-results" class="hidden mt-6">
        ...
        <div class="relative">
            <textarea id="ocr-text-output" rows="10" class="w-full bg-input-bg border border-gray-600 text-gray-300 rounded-lg p-2.5 font-sans" readonly></textarea>
...
`,
    ...
    <div id="signature-editor" class="hidden mt-6">
        <div class="bg-input-bg p-4 rounded-lg border border-gray-700 mb-4">
            <div class="flex border-b border-gray-700 mb-4">
                <button id="draw-tab-btn" class="flex-1 p-2 text-sm font-semibold border-b-2 border-fuchsia-500 text-white">Draw</button>
            ...
            <div id="type-panel" class="hidden">
                <input type="text" id="signature-text-input" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5 mb-4" placeholder="Type your name here">
                
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                    <div>
                        <label for="font-family-select" class="block mb-1 text-xs font-medium text-gray-400">Font Style</label>
                        <select id="font-family-select" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2 text-sm">
            ...
            <div id="upload-panel" class="hidden">
                <input type="file" id="signature-upload-input" accept="image/png" class="w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-fuchsia-600 file:text-white hover:file:bg-fuchsia-700">
                *png files only
            </div>
            
            <hr class="border-gray-700 my-4">
            <h4 class="text-md font-semibold text-white mb-2">Your Saved Signatures</h4>
            <div id="saved-signatures-container" class="flex flex-wrap gap-2 bg-tool-card p-2 rounded-md min-h-[50px]">
...
`,
    ...
    <div id="remove-annotations-options" class="hidden mt-6 space-y-6">
        <div>
            ...
            <div class="flex gap-4 p-2 rounded-lg bg-input-bg">
                <label class="flex-1 flex items-center gap-2 p-3 rounded-md hover:bg-gray-700 cursor-pointer">
                    <input type="radio" name="page-scope" value="all" checked class="w-4 h-4 text-fuchsia-600 bg-gray-700 border-gray-600 focus:ring-fuchsia-500">
                    <span class="font-semibold text-white">All Pages</span>
                </label>
                <label class="flex-1 flex items-center gap-2 p-3 rounded-md hover:bg-gray-700 cursor-pointer">
                    <input type="radio" name="page-scope" value="specific" class="w-4 h-4 text-fuchsia-600 bg-gray-700 border-gray-600 focus:ring-fuchsia-500">
                    <span class="font-semibold text-white">Specific Pages</span>
                </label>
            </div>
            <div id="page-range-wrapper" class="hidden mt-2">
                 <input type="text" id="page-range-input" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5" placeholder="e.g., 1-3, 5, 8">
            ...
            <div class="space-y-3 p-4 bg-gray-900 rounded-lg border border-gray-700">
                <div class="border-b border-gray-700 pb-2">
                    <label class="flex items-center gap-2 font-semibold text-white cursor-pointer">
                        <input type="checkbox" id="select-all-annotations" class="w-4 h-4 rounded text-fuchsia-600 bg-gray-700 border-gray-600">
                        Select / Deselect All
                    </label>
                </div>
...
`,
    ...
    <div id="cropper-ui-container" class="hidden mt-6">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between flex-wrap gap-4 mb-4 p-3 bg-input-bg rounded-lg border border-gray-700">
            <div class="flex items-center gap-2">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-4 flex-wrap">
                 <label class="flex items-center gap-2 text-sm font-medium text-gray-300">
                    <input type="checkbox" id="destructive-crop-toggle" class="w-4 h-4 rounded text-fuchsia-600 bg-gray-700 border-gray-600 focus:ring-fuchsia-500">
                    Enable Destructive Crop
                </label>
                 <label class="flex items-center gap-2 text-sm font-medium text-gray-300">
                    <input type="checkbox" id="apply-to-all-toggle" class="w-4 h-4 rounded text-fuchsia-600 bg-gray-700 border-gray-600 focus:ring-fuchsia-500">
                    Apply to all pages
                </label>
            </div>
        </div>
        ...
        <div id="cropper-container" class="w-full relative overflow-hidden flex items-center justify-center bg-(--background) rounded-lg border border-gray-600 min-h-[500px]"></div>
...
`,
    ...
    <div id="form-filler-options" class="hidden mt-6">
        <div class="flex flex-col lg:flex-row gap-4 h-[80vh]">
            ...
            <div class="w-full lg:w-1/3 bg-(--background) rounded-lg p-4 overflow-y-auto border border-gray-700 shrink-0">
            ...
            <div class="w-full lg:w-2/3 flex flex-col items-center gap-4">
                <div class="flex flex-nowrap items-center justify-center gap-4 p-3 bg-input-bg rounded-lg border border-gray-700">
...
`,
    ...
    <div id="posterize-options" class="hidden mt-6 space-y-6">
        ...
        <div class="space-y-2">
            ...
            <div id="posterize-preview-container" class="relative w-full max-w-xl mx-auto bg-(--background) rounded-lg border-2 border-gray-600 flex items-center justify-center">
        ...
        <div class="p-4 bg-(--background) border border-gray-700 rounded-lg">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" id="posterize-rows" value="1" min="1" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
                </div>
                <div>
                    <label for="posterize-cols" class="block mb-2 text-sm font-medium text-gray-300">Columns</label>
                    <input type="number" id="posterize-cols" value="2" min="1" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
                </div>
            </div>
        </div>
        ...
        <div class="p-4 bg-(--background) border border-gray-700 rounded-lg">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <div class="flex gap-4 p-2 rounded-lg bg-tool-card">
                        <label class="flex-1 flex items-center gap-2 p-3 rounded-md hover:bg-gray-700 cursor-pointer has-checked:bg-fuchsia-600">
                            <input type="radio" name="scaling-mode" value="fit" checked class="w-4 h-4 text-fuchsia-600 bg-gray-700 border-gray-600 focus:ring-fuchsia-500">
                            <div>
                                <span class="font-semibold text-white">Fit</span>
                 ...
                 <div>
                    ...
                    <div class="flex items-center gap-2">
                        <input type="number" id="overlap" value="0" min="0" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
                        <select id="overlap-units" class="bg-input-bg border border-gray-600 text-white rounded-lg p-2.5">
...
`,
    ...
    <div id="remove-blank-options" class="hidden mt-6 space-y-4">
        ...
        <div id="analysis-preview" class="hidden p-4 bg-input-bg border border-gray-700 rounded-lg">
...
`,
  ...
  `,
    ...
    <div id="attachment-options" class="hidden mt-8">
      ...
      <label for="attachment-files-input" class="w-full flex justify-center items-center px-6 py-10 bg-(--background) text-gray-400 rounded-lg border-2 border-dashed border-gray-600 hover:bg-gray-800 hover:border-gray-500 cursor-pointer transition-colors">
        <div class="text-center">
  ...
  `,
    ...
    <div id="sanitize-pdf-options" class="hidden mt-6 space-y-4 p-4 bg-(--background) border border-gray-700 rounded-lg">
    ...
    <div>
        ...
        <div class="mb-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <label class="flex items-center space-x-2 p-3 rounded-md bg-input-bg hover:bg-tool-card cursor-pointer">
                    <input type="checkbox" id="flatten-forms" name="sanitizeOption" value="flatten-forms" checked class="w-5 h-5 text-fuchsia-600 bg-gray-700 border-gray-600 rounded focus:ring-fuchsia-500">
                    <span class="text-white">Flatten Form Fields</span>
                </label>
                <label class="flex items-center space-x-2 p-3 rounded-md bg-input-bg hover:bg-tool-card cursor-pointer">
                    <input type="checkbox" id="remove-metadata" name="sanitizeOption" value="metadata" checked class="w-5 h-5 text-fuchsia-600 bg-gray-700 border-gray-600 rounded focus:ring-fuchsia-500">
                    <span class="text-white">Remove All Metadata</span>
                </label>
                <label class="flex items-center space-x-2 p-3 rounded-md bg-input-bg hover:bg-tool-card cursor-pointer">
                    <input type="checkbox" id="remove-annotations" name="sanitizeOption" value="annotations" checked class="w-5 h-5 text-fuchsia-600 bg-gray-700 border-gray-600 rounded focus:ring-fuchsia-500">
                    <span class="text-white">Remove Annotations</span>
                </label>
                <label class="flex items-center space-x-2 p-3 rounded-md bg-input-bg hover:bg-tool-card cursor-pointer">
                    <input type="checkbox" id="remove-javascript" name="sanitizeOption" value="javascript" checked class="w-5 h-5 text-fuchsia-600 bg-gray-700 border-gray-600 rounded focus:ring-fuchsia-500">
                    <span class="text-white">Remove JavaScript</span>
                </label>
                <label class="flex items-center space-x-2 p-3 rounded-md bg-input-bg hover:bg-tool-card cursor-pointer">
                    <input type="checkbox" id="remove-embedded-files" name="sanitizeOption" value="embeddedFiles" checked class="w-5 h-5 text-fuchsia-600 bg-gray-700 border-gray-600 rounded focus:ring-fuchsia-500">
                    <span class="text-white">Remove Embedded Files</span>
                </label>
                <label class="flex items-center space-x-2 p-3 rounded-md bg-input-bg hover:bg-tool-card cursor-pointer">
                    <input type="checkbox" id="remove-layers" name="sanitizeOption" value="layers" checked class="w-5 h-5 text-fuchsia-600 bg-gray-700 border-gray-600 rounded focus:ring-fuchsia-500">
                    <span class="text-white">Remove Layers (OCG)</span>
                </label>
                <label class="flex items-center space-x-2 p-3 rounded-md bg-input-bg hover:bg-tool-card cursor-pointer">
                    <input type="checkbox" id="remove-links" name="sanitizeOption" value="links" checked class="w-5 h-5 text-fuchsia-600 bg-gray-700 border-gray-600 rounded focus:ring-fuchsia-500">
        ...
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <label class="flex items-center space-x-2 p-3 rounded-md bg-input-bg hover:bg-tool-card cursor-pointer">
                    <input type="checkbox" id="remove-structure-tree" name="sanitizeOption" value="structure" class="w-5 h-5 text-fuchsia-600 bg-gray-700 border-gray-600 rounded focus:ring-fuchsia-500">
                    <span class="text-white">Remove Structure Tree</span>
                </label>
                <label class="flex items-center space-x-2 p-3 rounded-md bg-input-bg hover:bg-tool-card cursor-pointer">
                    <input type="checkbox" id="remove-markinfo" name="sanitizeOption" value="markinfo" class="w-5 h-5 text-fuchsia-600 bg-gray-700 border-gray-600 rounded focus:ring-fuchsia-500">
                    <span class="text-white">Remove Tagging Info</span>
                </label>
                <label class="flex items-center space-x-2 p-3 rounded-md bg-input-bg hover:bg-tool-card cursor-pointer">
                    <input type="checkbox" id="remove-fonts" name="sanitizeOption" value="fonts" class="w-5 h-5 text-fuchsia-600 bg-gray-700 border-gray-600 rounded focus:ring-fuchsia-500">
...
`,
  ...
  <div id="remove-restrictions-options" class="hidden space-y-4 mt-6">
      ...
      <div>
          ...
          <input type="password" id="owner-password-remove" class="w-full bg-input-bg border border-gray-600 text-white rounded-lg p-2.5" placeholder="Leave empty if PDF has no password">
