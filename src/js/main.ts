...
const init = () => {
  ...
  if (__SIMPLE_MODE__) {
    const hideBrandingSections = () => {
      ...
      const githubLink = document.querySelector('a[href*="github.com/alam00000/xtractor"]');
      if (githubLink) {
        ...
        simpleFooter.innerHTML = `
          <div class="container mx-auto px-4">
            <div class="flex items-center mb-4">
              <img src="images/favicon.svg" alt="Xtractor PDF Logo" class="h-8 w-8 mr-2">
              <span class="text-white font-bold text-lg">Xtractor</span>
            </div>
            <p class="text-gray-400 text-sm">
              &copy; 2025 Xtractor. All rights reserved.
            </p>
      ...
      document.title = 'Xtractor - PDF Tools';
  ...
  categories.forEach((category) => {
    ...
    title.className = 'text-xl font-bold text-fuchsia-400 mb-4 mt-8 first:mt-0';
    ...
    category.tools.forEach((tool) => {
      ...
      if (tool.href) {
        ...
        toolCard.className =
          'tool-card bg-[#18181b] rounded-xl p-4 cursor-pointer flex flex-col items-center justify-center text-center hover:shadow-lg transition duration-200';
        toolCard.dataset.toolId = tool.id;
      }

      const icon = document.createElement('i');
      icon.className = 'w-10 h-10 mb-3 text-fuchsia-400';
  ...
  if (githubStarsElement && !__SIMPLE_MODE__) {
    fetch('https://api.github.com/repos/motocaz/xtractor')
