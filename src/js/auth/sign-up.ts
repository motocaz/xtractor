...
export async function mountSignUp({
    containerId,
    onError
}: MountSignUpOptions): Promise<void> {
    try {
        await clerkReady;
        ...
        clerk.mountSignUp(container as HTMLDivElement, {
            routing: 'virtual',
        ...
        console.error('Error mounting Clerk sign-up component:', err);
        onError?.(err);
        throw err;
    }
}