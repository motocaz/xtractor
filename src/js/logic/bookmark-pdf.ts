...
function renderFileDisplay(file: File) {
  ...
  fileDiv.className =
    'flex items-center justify-between bg-tool-card border-fuchsia-700 p-3 rounded-lg text-sm';
...
exportCsvBtn.addEventListener('click', () => {
  ...
  a.download = `${now}-${originalFileName}.csv`;
  a.click();
...
exportJsonBtn.addEventListener('click', () => {
  ...
  a.href = url;
  const now = new Date().toISOString();
  a.download = `${now}_${originalFileName}.json`;
  a.click();
  URL.revokeObjectURL(url);
});
...
downloadBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    a.download = `${now} -${originalFileName}.pdf`;
    a.click();
    URL.revokeObjectURL(url);
