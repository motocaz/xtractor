...
export async function setupTxtToPdfTool() {
  ...
  const switchToUpload = () => {
    ...
    uploadBtn.classList.add('bg-fuchsia-600', 'text-white');
    uploadBtn.classList.remove('bg-gray-700', 'text-gray-300');
    textBtn.classList.remove('bg-fuchsia-600', 'text-white');
  ...
  const switchToText = () => {
    ...
    textBtn.classList.add('bg-fuchsia-600', 'text-white');
    textBtn.classList.remove('bg-gray-700', 'text-gray-300');
    uploadBtn.classList.remove('bg-fuchsia-600', 'text-white');
  ...
  try {
    ...
    if (isUploadMode && state.files.length > 0) {
      if (state.files.length === 1) {
        ...
        const now = new Date().toISOString();
        const baseName = state.files[0]?.name.replace(/\.txt$/i, '') || 'text-to-pdf';
