...
function updateFileList() {
  ...
  selectedFiles.forEach((file) => {
    ...
    fileDiv.className = 'flex items-center justify-between bg-tool-card border-fuchsia-700 p-3 rounded-lg text-sm mb-2'
...
worker.onmessage = async (e: MessageEvent) => {
  ...
  if (e.data.status === 'success') {
    ...
    try {
      ...
      const baseName = selectedFiles[0]?.name.replace(/\.json$/i, '') || 'jsons-to-pdf';
      const url = URL.createObjectURL(zipBlob)
      const a = document.createElement('a')
      a.href = url
      a.download = `${now}-${baseName}.zip`
