...
function initializeTool() {
  ...
  if (uploadArea) {
    uploadArea.addEventListener('dragover', (e) => {
      e.preventDefault();
      uploadArea.classList.add('border-fuchsia-500');
    });
    uploadArea.addEventListener('dragleave', () => {
      uploadArea.classList.remove('border-fuchsia-500');
    });
...
function createPageCard(pageData: PageData, index: number) {
  ...
  if (selectedPages.has(index)) {
    card.classList.add('border-fuchsia-500', 'ring-2', 'ring-fuchsia-500');
  }
  ...
  selectBtn.innerHTML = selectedPages.has(index)
    ? '<i data-lucide="check-square" class="w-4 h-4 text-fuchsia-400"></i>'
...
function toggleSelectOptimized(index: number) {
  ...
  if (selectedPages.has(index)) {
    card.classList.add('border-fuchsia-500', 'ring-2', 'ring-fuchsia-500');
    selectBtn.innerHTML = '<i data-lucide="check-square" class="w-4 h-4 text-fuchsia-400"></i>';
  } else {
    card.classList.remove('border-fuchsia-500', 'ring-2', 'ring-fuchsia-500');
...
async function bulkDownload() {
  ...
  const filename = originalFileNames[0] ? `${now}_${originalFileNames[0]}` : `${now}_selected-pages.pdf`;
  await downloadPagesAsPdf(indices, filename);
}
...
async function downloadAll() {
  ...
  if (splitMarkers.size > 0) {
    ...
    const now = new Date().toISOString();
    const filename = originalFileNames[0] ? `${now}_${originalFileNames[0]}` : `${now}_all-pages.pdf`;
    await downloadPagesAsPdf(indices, filename);
  }
}
...
async function downloadSplitPdfs() {
  try {
    ...
    const now = new Date().toISOString();
    const baseName = originalFileNames[0] ? originalFileNames[0].replace(/\.pdf$/i, '') : 'split-documents';
    downloadFile(zipBlob, undefined, `${now}_${baseName}.zip`);
